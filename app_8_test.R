...
# - for logs check:  rsconnect::showLogs("/Volumes/Transcend/Uni/doktorat/Predicting Core Beliefs/ShinyApp/PredictingVote/app_8_test.R")
# - for logs check:  rsconnect::showLogs("/Volumes/Transcend/Uni/doktorat/Predicting Core Beliefs/ShinyApp/PredictingVote backup/app_6.R")
# setwd("/Volumes/Transcend/Uni/doktorat/Predicting Core Beliefs/ShinyApp/PredictingVote")
# note: when using random forest, caret and ranger need to be loaded as libraries. Otherwise it might run locally but not on the server. Took me too long to figure out. 
...
server <- function(input, output) { 
    ...
    output$plot2 <- renderPlotly({
        ...
        ggplotly(
            dplyr::bind_cols(bind_rows(predict_probability(model_rate, ds())),
                             rep(c("Mobilisation"), each = nrow(predict_probability(model_rate, ds())))) %>%
